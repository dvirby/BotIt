...
class ManageGroups(BotFeature):
    ...
    def show_small_menu(self, group: Group, session: Session):
        ...
        buttons.append(self.ui.create_button_view("Change group description", lambda s: self.change_name(group, session)))
    ...
    def change_name(self, group, session):
        self.ui.create_text_view(session, "What ia the new group name?")
        self.ui.get_text(session, self.newName)

    def change_description(self, group, session):
        self.ui.create_text_view(session, "What ia the new group description?")
    ...
    def AddedA(self, session: Session, form_activity: FormActivity, form: AddGroupAdmins):
        ...
        new_participants = []
        for u in users:
            if u in group2 and u in form.AddedA.value:
                new_participants.append(u)
    ...
    def newName(self,text):
        global group2
        group2.name = text
        group2.save()

    def newDes(self, text):
        global group2
